...
class TopoMapper(object):
    ...
    def calcVertexKey(self, inOb):
        ...
        """
        try:
            coords = (inOb.Point.x, inOb.Point.y, inOb.Point.z)
        except:
            coords = (
                inOb.x,
                inOb.y,
        ...
        for value in coords:
            ...
            if keyPartial == "-000000000.000;":
                keyPartial = "0000000000.000;"
            key += keyPartial
        return key
    ...
    def calcAxisKey(self, axis):
        ...
        """
        coords = (axis.x, axis.y, axis.z)
        key = ""
        for value in coords:
            keyPartial = "%012.9f;" % value
            if keyPartial == "-0.000000000;":
                keyPartial = "00.000000000;"
            key += keyPartial
        return key
    ...
    def calcEdgeKeys(self, edge, pl):
        ...
        if (
            curveAttributeExists
            and hasattr(edge.Curve, "Axis")
            and hasattr(edge.Curve, "Radius")
        ):
            ...
            try:
                ...
                pass
            keys.append(self.calcVertexKey(endPoint1) + self.calcAxisKey(direction1))
            keys.append(self.calcVertexKey(endPoint2) + self.calcAxisKey(direction2))
        return keys
    ...
    def calcFaceKeys(self, face, pl):
        ...
        # a cylindric face...
            ...
            for v in face.Vertexes:
                ...
                keys.append("CYL;" + vertexKey + axisKey + radiusKey)
                keys.append("CYL;" + vertexKey + negativeAxisKey + radiusKey)
            ...
            for vert in face.Vertexes:
                ...
                keys.append("PLANE;" + vertexKey + normalKey)
                keys.append("PLANE;" + vertexKey + negativeNormalKey)
        else:
            keys.append("NOTRACE")
        return keys  # FIXME
    ...
    def populateShapeDict(self, objName):
        ...
        vertexNameSuffix = (
            str(numNewlyCreatedVertexes) + ";"
        )  # only correct for PartDesign, PartWB gives false counts
        ...
        edgeNameSuffix = (
            str(numNewlyCreatedEdges) + ";"
        )  # only correct for PartDesign, PartWB gives false counts
        i = 1  # do not enumerate the following, count new Edges !
        for edge in edges:
            edgeKeys = self.calcEdgeKeys(
                edge, pl
            )  # 2 keys for a linear edge, 1 key per circular edge
            entryFound = False
            for k in edgeKeys:
    ...
    def processTopoData(self, objName, level=0):
        ...
        if not objName.startswith("Body") and objName not in self.doneObjects:
    ...
    def isTopLevelInList(self, lst):
        if len(lst) == 0:
            return True
        for ob in lst:
            if ob.isDerivedFrom("PartDesign::SubShapeBinder"):
                continue
            if ob.isDerivedFrom("Part::SubShapeBinder"):
                continue
            if ob.Name.startswith("Group"):
                continue
            if ob.Name.startswith("ShapeBinder"):
                continue
            if ob.Name.startswith("Clone"):
                continue
            if ob.Name.startswith("Part__Mirroring"):
                continue
            if ob.Name.startswith("mirror"):
                continue
            if ob.hasExtension("App::GeoFeatureGroupExtension") and ob.Name.startswith(
                "Part"
            ):
                continue
            else:
    ...
    def getTopLevelObjects(self, allowSketches=False):
        ...
        for ob in S:
            ...
            self.treeNodes[ob.Name] = (inList, outList)
        # -------------------------------------------
        ...
        for objName in self.treeNodes.keys():
            ...
            elif allowSketches == True and objName.startswith(
                "Sketch"
            ):  # want to have all sketches
                self.topLevelShapes.append(objName)
            else:
                # -------------------------------------------
                ...
                if hasattr(ob, "ViewObject"):
                    if hasattr(ob.ViewObject, "Visibility"):
                        if (
                            ...
                            or not a2plib.isGlobalVisible(ob)
                        ):
                            print("Import ignored invisible shape! {}".format(ob.Name))
                            continue
            tmp.append(n)
    ...
    def getLinkedObjectRecursive(self, ob):
        if (
            ob.ViewObject.TypeId == "Gui::ViewProviderLinkPython"
        ):  # a link is involved...
            linkedObject = None
            ...
            for objName in self.topLevelShapes:
                ...
                if (
                    ...
                    needDiffuseExtension = len(
                        linkedObject.ViewObject.DiffuseColor
                    ...
                    needDiffuseExtension = len(ob.ViewObject.DiffuseColor) < len(
                        ob.Shape.Faces
                    )
