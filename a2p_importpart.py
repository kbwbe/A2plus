...
def importPartFromFile(
    ...
    if len(importableObjects) == 0:
        msg = "Aborting operation, no visible part was found."
    ...
    for dc_tx in my_list:
        ...
        if dc_tx is not None:
            newObj.sourcePart = dc_tx
        ...
        if subAssemblyImport:
            if extractSingleShape:
                newObj.muxInfo, newObj.Shape, newObj.ViewObject.DiffuseColor, newObj.ViewObject.Transparency = \
                    muxAssemblyWithTopoNames(importDoc,desiredShapeLabel = dc_tx)
            else:
                newObj.muxInfo, newObj.Shape, newObj.ViewObject.DiffuseColor, newObj.ViewObject.Transparency = \
            ...
            if extractSingleShape:
                newObj.muxInfo, newObj.Shape, newObj.ViewObject.DiffuseColor, newObj.ViewObject.Transparency = \
                    topoMapper.createTopoNames(desiredShapeLabel = dc_tx)
            else:
                newObj.muxInfo, newObj.Shape, newObj.ViewObject.DiffuseColor, newObj.ViewObject.Transparency = \
    ...
    def Activated(self):
        ...
        if FreeCAD.ActiveDocument.FileName == '':
            QtGui.QMessageBox.information(
                QtGui.QApplication.activeWindow(),
               "Unnamed document",
               '''The document has to have a name.\nSave the file before inserting the first part.'''
               )
...
'''
        ...
        importedObjects = importPartFromFile(doc, filename, extractSingleShape=True)

        if not importedObjects:
            a2plib.Msg("Imported object is empty\n")
            return
...
#==============================================================================
toolTip = \
'''Restore transparency on active document objects'''
...
class a2p_ImportPartCommand():
    ...
    def Activated(self):
        ...
        if not a2plib.checkFileIsInProjectFolder(filename):
            msg = \
'''The part you are importing is
outside of the project's folder.
Check your settings of A2plus preferences.
'''
            QtGui.QMessageBox.information(
                QtGui.QApplication.activeWindow(),
        ...
        # TODO: change for multi separate part import
        importedObjects = importPartFromFile(doc, filename)
...
toolTip = \
'''Duplicate an imported part. Holding "Shift" for multiple copies.'''
