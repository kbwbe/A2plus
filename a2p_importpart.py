...
class ObjectCache:
    ...
    def cleanUp(self, doc):
        for key in self.objects.keys():
            try:
                doc.removeObject(
                    self.objects[key].Name
                )  # remove temporaryParts from doc
            except:
                pass
...
class a2p_multiShapeExtractDialog(QtGui.QDialog):
    ...
    def initUI(self):
        ...
        self.connect(
            self.buttons, QtCore.SIGNAL("accepted()"), self, QtCore.SLOT("accept()")
        )
        self.connect(
            self.buttons, QtCore.SIGNAL("rejected()"), self, QtCore.SLOT("reject()")
        )
...
def importPartFromFile(
    ...
    if extractSingleShape:
        if desiredShapeLabel is None:  # ask for a shape label
            ...
            dialog = a2p_shapeExtractDialog(
                QtGui.QApplication.activeWindow(), labelList, dc
            )
            dialog.exec_()
            if dc.tx is None:
    ...
    if importToCache:
        ...
        if extractSingleShape == False:
            ...
            partLabel = a2plib.findUnusedObjectLabel(
                importDoc.Label, document=doc, extension=dc.tx
            )
        if PYVERSION < 3:
            newObj = doc.addObject("Part::FeaturePython", partName.encode("utf-8"))
        else:
            newObj = doc.addObject(
                "Part::FeaturePython", str(partName.encode("utf-8"))
            )  # works on Python 3.6.5
    ...
    if subAssemblyImport:
        if extractSingleShape:
            (
                ...
                newObj.ViewObject.Transparency,
    ...
    if importToCache:  # this import is used to update already imported parts
        ...
        if not a2plib.getPerFaceTransparency():
            ...
            newObj.ViewObject.Transparency = (
                0  # import assembly first time as non transparent.
            )
    ...
    if len(lcsList) > 0:
        ...
        if PYVERSION < 3:
            lcsGroup = doc.addObject(
                "Part::FeaturePython", lcsGroupObjectName.encode("utf-8")
            )
        else:
            lcsGroup = doc.addObject(
                "Part::FeaturePython", str(lcsGroupObjectName.encode("utf-8"))
            )  # works on Python 3.6.5
        ...
        newObj.addProperty(
            "App::PropertyLinkList", "lcsLink", "importPart"
        ).lcsLink = lcsGroup
...
class a2p_ImportShapeReferenceCommand:
    def GetResources(self):
        return {
            "Pixmap": a2plib.pathOfModule() + "/icons/a2p_ShapeReference.svg",
    ...
    def Activated(self):
        ...
        dialog.setNameFilter(
            "Supported Formats (*.FCStd *.fcstd *.stp *.step);;All files (*.*)"
        )
        if dialog.exec_():
            if PYVERSION < 3:
...
"""
        ...
        if not importDocIsOpen:
            if filename.lower().endswith(".fcstd"):
                importDoc = FreeCAD.openDocument(filename)
            elif filename.lower().endswith(".stp") or filename.lower().endswith(
                ".step"
            ):
                import ImportGui
        ...
        dialog = a2p_multiShapeExtractDialog(
            QtGui.QApplication.activeWindow(), labelList, iconList, dc
        )
        dialog.exec_()
        ...
        for so in selectedObjects:
            importedObject = importPartFromFile(
                doc, filename, extractSingleShape=True, desiredShapeLabel=so
            )
        ...
        try:
            FreeCAD.closeDocument(
                importDoc.Name
            )  # avoid errormessage if doc already closed...
        except:
            pass
        ...
        for io in importedObjectsList:
            io.ViewObject.Visibility = (
                True  # make imported objects visible step by step,
            )
...
FreeCADGui.addCommand(
    "a2p_ImportShapeReferenceCommand", a2p_ImportShapeReferenceCommand()
)
...
class a2p_Restore_Transparency_Command:
    def GetResources(self):
        return {
            "Pixmap": a2plib.pathOfModule() + "/icons/a2p_Restore_Transparency.svg",
            "Accel": "Shift+T",  # a default shortcut (optional)
            "MenuText": QT_TRANSLATE_NOOP(
                "A2plus_importpart", "Restore transparency to active document objects"
            ),
            "ToolTip": toolTip,
        }
...
class a2p_ImportPartCommand:
    def GetResources(self):
        return {
            "Pixmap": a2plib.pathOfModule() + "/icons/a2p_ImportPart.svg",
            "Accel": "Shift+A",  # a default shortcut (optional)
            "MenuText": QT_TRANSLATE_NOOP(
                "A2plus_importpart", "Add a part from an external file"
            ),
            "ToolTip": toolTip,
        }
...
"""
        ...
        if importedObject and not importedObject.fixedPosition:
            ...
            QtCore.QObject.connect(
                self.timer, QtCore.SIGNAL("timeout()"), self.GuiViewFit
            )
            self.timer.start(200)  # 0.2 seconds
        return
...
def updateImportedParts(doc, partial=False):
    ...
    selection = [
        ...
        and (a2plib.isA2pPart(s) or a2plib.isA2pSketch(s))
    ]
    if selection and len(selection) > 0:
        if partial == True:
            ...
            response = QtGui.QMessageBox.information(
                QtGui.QApplication.activeWindow(), u"ASSEMBLY UPDATE", msg, flags
            )
    ...
    for obj in workingSet:
        if hasattr(obj, "sourceFile") and a2plib.to_str(
            obj.sourceFile
        ) == a2plib.to_str("converted"):
            if (
                hasattr(obj, "localSourceObject")
                and obj.localSourceObject is not None
                and obj.localSourceObject != ""
            ):
        ...
        if hasattr(obj, "sourceFile") and a2plib.to_str(
            ...
            if (
                hasattr(obj, "Content") and "importPart" in obj.Content
            ):  # be sure to have an assembly object
            ...
            if absPath != None and os.path.exists(absPath):
                ...
                if (
                    newPartCreationTime > obj.timeLastImport
                    or obj.a2p_Version != A2P_VERSION
                    or a2plib.getRecalculateImportedParts()  # open always all parts as they could depend on spreadsheets
                ):
                    ...
                    importUpdateConstraintSubobjects(
                        doc, obj, newObject
                    )  # do this before changing shape and mux
...
class a2p_UpdateImportedPartsCommand:
    ...
    def GetResources(self):
        return {
            ...
            "MenuText": QT_TRANSLATE_NOOP(
                "A2plus_importpart", "Update parts imported into the assembly"
            ),
            "ToolTip": toolTip,
        }
...
def duplicateImportedPart(part):
    ...
    for (
        ...
        part.ViewObject.PropertiesList
    ):  # assuming that the user may change the appearance of parts differently depending on their role in the assembly.
        if hasattr(part.ViewObject, p) and p not in [
            "DiffuseColor",
            "Proxy",
            "MappedColors",
        ]:
...
class a2p_DuplicatePartCommand:
    ...
    def Activated(self):
        doc = FreeCAD.activeDocument()
        selection = [
            s for s in FreeCADGui.Selection.getSelectionEx() if s.Document == doc
        ]
        ...
        if self.partMover != None:
            if self.partMover.objectToDelete != None:
                FreeCAD.activeDocument().removeObject(
                    self.partMover.objectToDelete.Name
                )
    ...
    def GetResources(self):
        return {
            ...
            "MenuText": QT_TRANSLATE_NOOP(
                "A2plus_importpart", "Create duplicate of a part"
            ),
            "ToolTip": toolTip,
        }
...
"""
            QtGui.QMessageBox.critical(
                QtGui.QApplication.activeWindow(), "File error ! ", msg
            )
            return
        ...
        if not importDocIsOpen:
            if fileNameWithinProjectFile.lower().endswith(
                ".stp"
            ) or fileNameWithinProjectFile.lower().endswith(".step"):
                import ImportGui
                ...
                FreeCADGui.SendMsgToActiveView("ViewFit")
                msg = (
                    "Editing a STEP file as '*.FCStd' file\nPlease export the saved file as '.step'\n"
                    + fileNameWithinProjectFile
                )
                QtGui.QMessageBox.information(
                    QtGui.QApplication.activeWindow(), "Info", msg
                )
            else:
            ...
            for s in sub:
                ...
                if FreeCAD.activeDocument().Name == name:
                    break
    ...
    def IsActive(self):
        ...
        if not a2plib.isEditableA2pPart(selection[0]):
            return False
    ...
    def GetResources(self):
        return {
            ...
            "MenuText": QT_TRANSLATE_NOOP(
                "A2plus_importpart", "Edit an imported part (open linked FCStd file)"
            ),
            "ToolTip": toolTip,
        }
...
class a2p_MovePartCommand:
    ...
    def GetResources(self):
        return {
            ...
            "MenuText": QT_TRANSLATE_NOOP(
                "A2plus_importpart", "Move the selected part"
            ),
            "ToolTip": toolTip,
        }
...
class ConstrainedPartsMover:
    def __init__(self, view):
        ...
        self.callbackMove = self.view.addEventCallback(
            "SoLocation2Event", self.onMouseMove
        )
        self.callbackClick = self.view.addEventCallback(
            "SoMouseButtonEvent", self.onMouseClicked
        )
        self.callbackKey = self.view.addEventCallback(
            "SoKeyboardEvent", self.KeyboardEvent
        )
    ...
    def onMouseMove(self, info):
        ...
        if self.motionActivated:
            ...
            systemSolved = a2p_solversystem.solveConstraints(
                self.doc, useTransaction=False
            )
...
class a2p_MovePartUnderConstraints:
    ...
    def Activated(self):
        self.partMover = ConstrainedPartsMover(FreeCADGui.activeDocument().activeView())
    ...
    def GetResources(self):
        return {
            ...
            "MenuText": QT_TRANSLATE_NOOP(
                "A2plus_importpart", "Move the selected part under constraints"
            ),
            "ToolTip": toolTip,
        }
...
class DeleteConnectionsCommand:
    def Activated(self):
        ...
        if len(deleteList) == 0:
            ...
            response = QtGui.QMessageBox.information(
                QtGui.QApplication.activeWindow(), "Delete constraints?", msg, flags
            )
            if response == QtGui.QMessageBox.Yes:
                doc = FreeCAD.activeDocument()
    ...
    def GetResources(self):
        return {
            ...
            "MenuText": QT_TRANSLATE_NOOP(
                "A2plus_importpart", "Delete all constraints of selected parts"
            ),
            "ToolTip": toolTipText,
        }
...
class ViewConnectionsCommand:
    def Activated(self):
        ...
        FreeCADGui.Selection.addObserver(
            ViewConnectionsObserver(initialTransparencyState)
        )
    ...
    def GetResources(self):
        return {
            ...
            "MenuText": QT_TRANSLATE_NOOP(
                "A2plus_importpart", "Highlight both constrained parts"
            ),
            "ToolTip": toolTip,
        }
...
class a2p_isolateCommand:
    def hasFaces(self, ob):
        if (
            hasattr(ob, "Shape")
            and hasattr(ob.Shape, "Faces")
            and len(ob.Shape.Faces) > 0
        ):
            return True
        return False
    ...
    def Activated(self):
        ...
        if len(selection) == 0:  # Show all elements
            for obj in doc.Objects:
                if obj.Name == "PartInformation":
                    continue
                if obj.Name[:4] == "Page":
                    continue
                if obj.Name == "SimpleAssemblyShape":
                    continue
                if not self.hasFaces(obj):
                    continue
                if hasattr(obj, "ViewObject"):
...
class a2p_ToggleTransparencyCommand:
    ...
    def GetResources(self):
        return {
            ...
            "MenuText": QT_TRANSLATE_NOOP(
                "A2plus_importpart", "Toggle transparency of assembly"
            ),
            "ToolTip": QT_TRANSLATE_NOOP(
                "A2plus_importpart", "Toggles transparency of assembly"
            ),
...
class a2p_TogglePartialProcessingCommand:
    ...
    def GetResources(self):
        return {
            ...
            "MenuText": QT_TRANSLATE_NOOP(
                "A2plus_importpart", "Toggle partial processing"
            ),
            "ToolTip": QT_TRANSLATE_NOOP(
                "A2plus_importpart", "Toggle partial processing"
            ),
...
FreeCADGui.addCommand(
    "a2p_TogglePartialProcessingCommand", a2p_TogglePartialProcessingCommand()
)
...
class a2p_repairTreeViewCommand:
    ...
    def GetResources(self):
        return {
            ...
            "MenuText": QT_TRANSLATE_NOOP(
                "A2plus_importpart", "Repair the tree view if it is somehow damaged"
            ),
            "ToolTip": toolTipMessage,
        }
...
class a2p_FlipConstraintDirectionCommand:
    ...
    def GetResources(self):
        return {
            ...
            "MenuText": QT_TRANSLATE_NOOP(
                "A2plus_importpart", "Flip direction of last constraint"
            ),
            "ToolTip": toolTip,
        }
...
FreeCADGui.addCommand(
    "a2p_FlipConstraintDirectionCommand", a2p_FlipConstraintDirectionCommand()
)
...
def a2p_FlipConstraintDirection():
    ...
    constraints = [
        obj for obj in FreeCAD.ActiveDocument.Objects if "ConstraintInfo" in obj.Content
    ]
    ...
    try:
        ...
        a2p_solversystem.autoSolveConstraints(
            FreeCAD.activeDocument(), callingFuncName="a2p_FlipConstraintDirection"
        )
    except:
        pass
...
class a2p_Show_Hierarchy_Command:
    ...
    def GetResources(self):
        return {
            ...
            "ToolTip": QT_TRANSLATE_NOOP(
                "A2plus_importpart",
                "Generates HTML file with detailed constraining structure",
            ),
        }
...
class a2p_Show_PartLabels_Command:
    ...
    def GetResources(self):
        return {
            ...
            "MenuText": QT_TRANSLATE_NOOP(
                "A2plus_importpart", "Show part labels in 3D view"
            ),
            "ToolTip": QT_TRANSLATE_NOOP(
                "A2plus_importpart", "Toggle showing part labels in 3D view"
            ),
            "Checkable": False,
        }
...
class a2p_Show_DOF_info_Command:
    ...
    def GetResources(self):
        return {
            ...
            "MenuText": QT_TRANSLATE_NOOP(
                "A2plus_importpart", "Print detailed DOF information"
            ),
            "ToolTip": QT_TRANSLATE_NOOP(
                "A2plus_importpart", "Toggle printing detailed DOF information"
            ),
            "Checkable": False,
        }
...
class a2p_absPath_to_relPath_Command:
    ...
    def GetResources(self):
        return {
            ...
            "MenuText": QT_TRANSLATE_NOOP(
                "A2plus_importpart",
                "Convert absolute paths of imported parts to relative ones",
            ),
            "ToolTip": QT_TRANSLATE_NOOP(
                "A2plus_importpart",
                "Converts absolute paths of imported parts to relative ones",
            ),
        }


FreeCADGui.addCommand(
    "a2p_absPath_to_relPath_Command", a2p_absPath_to_relPath_Command()
)
...
class a2p_SaveAndExit_Command:
    ...
    def GetResources(self):
        return {
            ...
            "MenuText": QT_TRANSLATE_NOOP(
                "A2plus_importpart", "Save and exit the active document"
            ),
            "ToolTip": QT_TRANSLATE_NOOP(
                "A2plus_importpart", "Save and exit the active document"
            ),
...
class a2p_MigrateProxiesCommand:
        ...
        if response == QtGui.QMessageBox.Yes:
            ...
            for ob in doc.Objects:
                if a2plib.isA2pPart(ob):
                    ...
                    if FreeCAD.GuiUp:
                        a2p_importedPart_class.ImportedPartViewProviderProxy(
                            ob.ViewObject
                        )
    ...
    def GetResources(self):
        return {
            ...
            "MenuText": QT_TRANSLATE_NOOP(
                "A2plus_importpart", "Migrate proxies of imported parts"
            ),
            "ToolTip": toolTip,
        }
...
def importUpdateConstraintSubobjects(doc, oldObject, newObject):
    ...
    if len(deletionList) > 0:  # there are broken constraints..
        for cName in deletionList:
            ...
            message = (
                "Constraint %s is broken. Delete constraint? Otherwise check for wrong linkage."
                % cName
            )
            ...
            response = QtGui.QMessageBox.critical(
                None, "Broken Constraint", message, flags
            )
...
class a2p_cleanUpDebug3dCommand:
    def Activated(self):
        ...
        if sg is not None:
            print(
                "3D-Debug contained {} vectors".format(len(a2plib.solver_debug_objects))
            )
    ...
    def GetResources(self):
        return {
            ...
            "MenuText": QT_TRANSLATE_NOOP(
                "A2plus_importpart", "Clean up solver debug output from 3D view"
            ),
            "ToolTip": toolTip,
        }
